...
class A3ImageButton : AppCompatImageButton {
    ...
    private fun build(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        val ta: TypedArray = context.theme.obtainStyledAttributes(attrs, R.styleable.A3ImageButton, defStyleAttr, 0)

        if (ta.hasValue(R.styleable.A3ImageButton_a3_square) && square == -1)
            square = ta.getInt(R.styleable.A3ImageButton_a3_square, 0)
        if (square < 0) square = 0

        if (ta.hasValue(R.styleable.A3ImageButton_a3_squareSize) && squareSize == -1)
            squareSize = ta.getDimensionPixelSize(R.styleable.A3ImageButton_a3_squareSize, 0)

        if (ta.hasValue(R.styleable.A3ImageButton_a3_directionable))
            isDirectionable = ta.getBoolean(R.styleable.A3ImageButton_a3_directionable, false)
