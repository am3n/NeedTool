package ir.am3n.needtool.views.nspicker;
 ...
 */
public class NSPicker extends LinearLayout {
    ...
    /**
     * Use a custom NSPicker formatting callback to use two-digit minutes
    ...
    /**
     * Lower value of the range of numbers allowed for the NSPicker
     */
    private int mMinValue = DEFAULT_MIN_VALUE;

    /**
     * Upper value of the range of numbers allowed for the NSPicker
     */
    private int mMaxValue = DEFAULT_MAX_VALUE;

    /**
     * Current value of this NSPicker
     */
    private int mValue;
    ...
    public interface OnValueChangeListener {
         ...
         *
         * @param picker The NSPicker associated with this listener.
         * @param oldVal The previous value.
         * @param newVal The new value.
         */
        void onValueChange(NSPicker picker, int oldVal, int newVal);
    }
    ...
    public interface OnScrollListener {
         ...
         *                    {@link #SCROLL_STATE_IDLE}.
         */
        public void onScrollStateChange(NSPicker view, @ScrollState int scrollState);
    }
     ...
     */
    public NSPicker(Context context) {
        this(context, null);
    }
     ...
     */
    public NSPicker(Context context, AttributeSet attrs) {
     ...
     */
    public NSPicker(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs);
        mContext = context;
        mNumberFormatter = NumberFormat.getInstance();

        final TypedArray attr = context.obtainStyledAttributes(attrs, R.styleable.NSPicker, 0, 0);

        final Drawable selectionDivider = attr.getDrawable(R.styleable.NSPicker_nsp_divider);
        if (selectionDivider != null) {
            ...
            mDividerColor = attr.getColor(R.styleable.NSPicker_nsp_dividerColor, mDividerColor);
        ...
        mDividerDistance = attr.getDimensionPixelSize(R.styleable.NSPicker_nsp_dividerDistance, defDividerDistance);
        mDividerLength = attr.getDimensionPixelSize(R.styleable.NSPicker_nsp_dividerLength, 0);
        mDividerThickness = attr.getDimensionPixelSize(R.styleable.NSPicker_nsp_dividerThickness, defDividerThickness);
        mDividerType = attr.getInt(R.styleable.NSPicker_nsp_dividerType, SIDE_LINES);

        mOrder = attr.getInt(R.styleable.NSPicker_nsp_order, ASCENDING);
        mOrientation = attr.getInt(R.styleable.NSPicker_nsp_orientation, VERTICAL);

        final float width = attr.getDimensionPixelSize(R.styleable.NSPicker_nsp_width, SIZE_UNSPECIFIED);
        final float height = attr.getDimensionPixelSize(R.styleable.NSPicker_nsp_height, SIZE_UNSPECIFIED);

        setWidthAndHeight();

        mComputeMaxWidth = true;

        mValue = attr.getInt(R.styleable.NSPicker_nsp_value, mValue);
        mMaxValue = attr.getInt(R.styleable.NSPicker_nsp_max, mMaxValue);
        mMinValue = attr.getInt(R.styleable.NSPicker_nsp_min, mMinValue);

        mSelectedTextAlign = attr.getInt(R.styleable.NSPicker_nsp_selectedTextAlign, mSelectedTextAlign);
        mSelectedTextColor = attr.getColor(R.styleable.NSPicker_nsp_selectedTextColor, mSelectedTextColor);
        mSelectedTextSize = attr.getDimension(R.styleable.NSPicker_nsp_selectedTextSize, spToPx(mSelectedTextSize));
        mSelectedTextStrikeThru = attr.getBoolean(R.styleable.NSPicker_nsp_selectedTextStrikeThru, mSelectedTextStrikeThru);
        mSelectedTextUnderline = attr.getBoolean(R.styleable.NSPicker_nsp_selectedTextUnderline, mSelectedTextUnderline);
        try {
            mSelectedTypeface = ResourcesCompat.getFont(context, attr.getResourceId(R.styleable.NSPicker_nsp_selectedTypeface, 0));
        } catch (Throwable t) {
            t.printStackTrace();
        }
        mTextAlign = attr.getInt(R.styleable.NSPicker_nsp_textAlign, mTextAlign);
        mTextColor = attr.getColor(R.styleable.NSPicker_nsp_textColor, mTextColor);
        mTextSize = attr.getDimension(R.styleable.NSPicker_nsp_textSize, spToPx(mTextSize));
        mTextStrikeThru = attr.getBoolean(R.styleable.NSPicker_nsp_textStrikeThru, mTextStrikeThru);
        mTextUnderline = attr.getBoolean(R.styleable.NSPicker_nsp_textUnderline, mTextUnderline);
        try {
            mTypeface = ResourcesCompat.getFont(context, attr.getResourceId(R.styleable.NSPicker_nsp_typeface, 0));
        } catch (Throwable t) {
            t.printStackTrace();
        }
        mFormatter = stringToFormatter(attr.getString(R.styleable.NSPicker_nsp_formatter));
        mFadingEdgeEnabled = attr.getBoolean(R.styleable.NSPicker_nsp_fadingEdgeEnabled, mFadingEdgeEnabled);
        mFadingEdgeStrength = attr.getFloat(R.styleable.NSPicker_nsp_fadingEdgeStrength, mFadingEdgeStrength);
        mScrollerEnabled = attr.getBoolean(R.styleable.NSPicker_nsp_scrollerEnabled, mScrollerEnabled);
        mWheelItemCount = attr.getInt(R.styleable.NSPicker_nsp_wheelItemCount, mWheelItemCount);
        mLineSpacingMultiplier = attr.getFloat(R.styleable.NSPicker_nsp_lineSpacingMultiplier, mLineSpacingMultiplier);
        mMaxFlingVelocityCoefficient = attr.getInt(R.styleable.NSPicker_nsp_maxFlingVelocityCoefficient, mMaxFlingVelocityCoefficient);
        mHideWheelUntilFocused = attr.getBoolean(R.styleable.NSPicker_nsp_hideWheelUntilFocused, false);
        mAccessibilityDescriptionEnabled = attr.getBoolean(R.styleable.NSPicker_nsp_accessibilityDescriptionEnabled, true);
        mItemSpacing = attr.getDimensionPixelSize(R.styleable.NSPicker_nsp_itemSpacing, 0);
        ...
        inflater.inflate(R.layout.ns_picker_material, this, true);

        // input text
        mSelectedText = findViewById(R.id.ns_picker_input);
        ...
        mWrapSelectorWheel = attr.getBoolean(R.styleable.NSPicker_nsp_wrapSelectorWheel, mWrapSelectorWheel);
        ...
        mMaximumFlingVelocity = mViewConfiguration.getScaledMaximumFlingVelocity() / mMaxFlingVelocityCoefficient;
     ...
     * <p>
     * If the argument is less than the {@link NSPicker#getMinValue()} and
     * {@link NSPicker#getWrapSelectorWheel()} is <code>false</code> the
     * current value is set to the {@link NSPicker#getMinValue()} value.
     * </p>
     * <p>
     * If the argument is less than the {@link NSPicker#getMinValue()} and
     * {@link NSPicker#getWrapSelectorWheel()} is <code>true</code> the
     * current value is set to the {@link NSPicker#getMaxValue()} value.
     * </p>
     * <p>
     * If the argument is less than the {@link NSPicker#getMaxValue()} and
     * {@link NSPicker#getWrapSelectorWheel()} is <code>false</code> the
     * current value is set to the {@link NSPicker#getMaxValue()} value.
     * </p>
     * <p>
     ...
     * wrap around the {@link NSPicker#getMinValue()} and
     * {@link NSPicker#getMaxValue()} values.
     * <p>
...
//        }
    ...
    public void setDisplayedValues(String[] displayedValues) {
        ...
        if (mDisplayedValues != null) {
            ...
            mSelectedText.setRawInputType(InputType.TYPE_TEXT_FLAG_MULTI_LINE | InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS);
        } else {
    ...
    protected void drawableStateChanged() {
        ...
        if (mDividerDrawable != null && mDividerDrawable.isStateful() && mDividerDrawable.setState(getDrawableState())) {
    ...
    private void drawHorizontalDividers(Canvas canvas) {
        switch (mDividerType) {
            ...
            case UNDERLINE:
                ...
                mDividerDrawable.setBounds(left, topOfUnderlineDivider, right, bottomOfUnderlineDivider);
                mDividerDrawable.draw(canvas);
                break;
        }
    }
    ...
    private void drawVerticalDividers(Canvas canvas) {
        ...
        switch (mDividerType) {
            case SIDE_LINES:
                ...
                mDividerDrawable.setBounds(left, topOfBottomDivider, right, bottomOfBottomDivider);
                mDividerDrawable.draw(canvas);
                break;
            case UNDERLINE:
                final int bottomOfUnderlineDivider = mBottomDividerBottom;
    ...
    private void drawText(String text, float x, float y, Paint paint, Canvas canvas) {
        if (text.contains("\n")) {
            ...
            final float height = Math.abs(paint.descent() + paint.ascent()) * mLineSpacingMultiplier;
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        event.setClassName(NSPicker.class.getName());
    ...
    /**
     * Sets the current value of this NSPicker.
     *
     * @param current      The new value of the NSPicker.
    ...
    /**
     * Updates the view of this NSPicker. If displayValues were specified in
