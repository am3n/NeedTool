...
public class CircleCheckBox extends View implements Checkable {
    ...
    private void init(AttributeSet attrs) {
        ...
        TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.CircleCheckBox);
        mTickColor = ta.getColor(R.styleable.CircleCheckBox_ccb_colorTick, COLOR_TICK);
        mAnimDuration = ta.getInt(R.styleable.CircleCheckBox_ccb_duration, DEF_ANIM_DURATION);
        mFloorColor = ta.getColor(R.styleable.CircleCheckBox_ccb_colorUncheckedStroke, COLOR_FLOOR_UNCHECKED);
        mCheckedColor = ta.getColor(R.styleable.CircleCheckBox_ccb_colorChecked, COLOR_CHECKED);
        mUnCheckedColor = ta.getColor(R.styleable.CircleCheckBox_ccb_colorUnchecked, COLOR_UNCHECKED);
        mStrokeWidth = ta.getDimensionPixelSize(R.styleable.CircleCheckBox_ccb_strokeWidth, dp2px(getContext(), 0));
        mSmallTick = ta.getBoolean(R.styleable.CircleCheckBox_ccb_smallTick, false);
        ta.recycle();
